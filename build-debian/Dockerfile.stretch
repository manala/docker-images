...
# Packages
COPY apt/sources.stretch     /etc/apt/sources.list
COPY apt/preferences.stretch /etc/apt/preferences.d/preferences
RUN apt-get update && \
    DEBIAN_FRONTEND=noninteractive apt-get -y --no-install-recommends install \
      apt-utils apt-transport-https gnupg dirmngr
COPY apt/sources.manala.stretch /etc/apt/sources.list.d/manala.list
